...
class KtlintReportHandler : ReportHandler<JavaExec>() {

  private val failures = ArrayList<Failure>()
  private var file: File? = null

  override fun configureTask(task: JavaExec) {
    var outputDir = KtlintUtils.getOutputDir(task)

    if (outputDir == null) {
      outputDir = KtlintUtils.getReportDir(task.project)
      task.args = listOf("--reporter=checkstyle,output=$outputDir") + task.args.orEmpty()
    }
  }
